...
public class DataService
{
    ...
    public void Update(string id, Func<UpdateDefinitionBuilder<Download>, UpdateDefinition<Download>> updateFunction)
    {
    ...
    public async void Insert(Download downloadToInsert)
    {
    ...
    public void RebuildCache(IMongoCollection<Download> downloadCollection = null)
    {
        ...
        using (this.distributedLockService.Acquire(0, $"{this.redisDownloadListKey}-key", TimeSpan.FromSeconds(15)))
        {
            ...
            var list = mongoList.ConvertAll(it => (RedisValue)Newtonsoft.Json.JsonConvert.SerializeObject(it));
